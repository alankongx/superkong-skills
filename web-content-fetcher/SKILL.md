...
```bash
# 当前安装版本（2026-05 验证可用）
python3.11 -m pip install "scrapling[fetchers]" html2text
scrapling install  # 刷新浏览器指纹
...
- 当前安装：`scrapling==0.4.8` + `html2text==2025.4.15`，Python 3.11.9 (arm64)
- `scrapling[fetchers]` 完整包含 playwright / patchright / curl_cffi 等浏览器依赖
- 浏览器指纹已通过 `scrapling install --force` 刷新（2026-05-26）
- 当前组合已实测可成功提取微信公众号文章
...
调用方式：
```bash
python3.11 ~/.easyclaw/skills/web-content-fetcher/scripts/fetch.py <url> [max_chars]
```
