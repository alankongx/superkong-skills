...
- Provide API key via either:
  ...
  - `~/.easyclaw/.env` line: `TAVILY_API_KEY=...`

## Commands

Run from the EasyClaw workspace:

```bash
# raw JSON (default)
python {baseDir}/scripts/tavily_search.py --query "..." --max-results 5

# include short answer (if available)
python {baseDir}/scripts/tavily_search.py --query "..." --max-results 5 --include-answer

# stable schema (closer to web_search): {query, results:[{title,url,snippet}], answer?}
python {baseDir}/scripts/tavily_search.py --query "..." --max-results 5 --format brave

# human-readable Markdown list
python {baseDir}/scripts/tavily_search.py --query "..." --max-results 5 --format md
```
